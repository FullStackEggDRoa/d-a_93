...
```
 SELECT nombre AS "Nombre Fabricante" FROM fabricante WHERE codigo IN(SELECT codigo_fabricante FROM producto GROUP BY codigo_fabricante HAVING COUNT(codigo_fabricante) = (SELECT COUNT(codigo) FROM producto WHERE codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = "Lenovo")));
+-------------------+
| Nombre Fabricante |
+-------------------+
| Asus              |
